...
use vector_lib::{
    ...
    counter,
    event::BatchNotifier,
    internal_event::{CountByteSize, CounterName, InternalEventHandle as _, Registered},
...
const MAX_RAW_LINE_EVENTS_PER_REQUEST: usize = 10_000;
...
pub struct SplunkHecFullConfig {
    ...
    /// Legacy compatibility option that copies HEC metadata fields (`index`,
    /// `sourcetype`, `source`, and `host`) into ordinary event fields.
    ///
    /// This can mutate transparent payloads. Prefer reading source metadata
    /// directly from downstream templates such as
    /// `index: "{{ %splunk_hec_full.index }}"`,
    /// `source: "{{ %splunk_hec_full.source }}"`, and
    /// `sourcetype: "{{ %splunk_hec_full.sourcetype }}"`.
    #[serde(default = "default_false")]
    ...
    /// by newline characters (`\n`) into separate events. Empty lines are
    /// skipped. This is newline-delimited raw handling, not a full Splunk
    /// `LINE_BREAKER` implementation.
    ///
...
impl HealthQueryParams {
    ...
    fn ack_enabled(&self) -> bool {
        self.ack.as_deref().is_some_and(|v| v == "true" || v == "1")
    }
}
...
impl SplunkSource {
    ...
    fn event_service(&self, out: SourceSender) -> BoxedFilter<(Response,)> {
        ...
        let event_query_params = warp::query::<EventQueryParams>().or_else(|_| async {
            Ok::<(EventQueryParams,), Rejection>((EventQueryParams::default(),))
        });

        let splunk_channel = splunk_channel_header.and(event_query_params).map(
            |header: Option<String>, mut params: EventQueryParams| {
                let auto_extract_ts = params.auto_extract_timestamp_enabled();
                let channel = header.or(params.channel.take());
                (channel, auto_extract_ts, params)
            },
        );
        ...
        warp::post()
            ...
            .and_then(
                move |_,
                      token: Option<String>,
                      (channel, auto_extract_ts, query_params): (
                    Option<String>,
                    bool,
                    EventQueryParams,
                ),
                    ...
                    async move {
                        ...
                        let (byte_size, body) = if gzip {
                            data = CappedDecoder::gzip(body.reader()).decompress().map_err(
                                |error| {
                                    diagnostic_context.emit(
                                        ApiError::BadRequest,
                                        channel.as_deref(),
                                    ...
                                    gzip_decompression_rejection(&error, ApiError::BadRequest)
                                },
                            )?;
                        ...
                        let iter: EventIterator<'_, StrRead<'_>> = EventIteratorGenerator {
                            ...
                            channel,
                            query_host: query_params.host,
                            query_index: query_params.index,
                            query_source: query_params.source,
                            query_sourcetype: query_params.sourcetype,
                            query_time: query_params.time,
                            remote,
                        ...
                        for event in events.iter_mut() {
                            ...
                            insert_hec_metadata(
                                ...
                                log,
                                HecMetadata {
                                    endpoint: Some(Value::from("event")),
                                    ..Default::default()
                                },
                                false,
                            )?;
                        ...
                        let channel_id = match channel_id {
                            ...
                            None => {
                                if raw_require_channel {
                                    let body_for_diagnostic =
                                        diagnostic_body(&body, gzip, bad_request_diagnostics);
                                    diagnostic_context.emit(
                                        ApiError::MissingChannel,
                                        None,
                                        body.len(),
                        ...
                        let maybe_ack_id = match (idx_ack, receiver) {
                            (Some(idx_ack), Some(receiver)) => Some(
                                idx_ack
                                    ...
                                    .inspect_err(|rej| {
                                        if let Some(&error) = rej.find::<ApiError>() {
                                            let body_for_diagnostic = diagnostic_body(
                                                &body,
                                                gzip,
                        ...
                        for event in events.iter_mut() {
                            {
                                ...
                                insert_hec_metadata(
                                    ...
                                    log,
                                    HecMetadata {
                                        host: final_host.clone().map(Value::from),
                                        index: final_index.clone().map(Value::from),
                                        source: final_source.clone().map(Value::from),
                                        sourcetype: final_sourcetype.clone().map(Value::from),
                                        endpoint: Some(Value::from("raw")),
                                        ..Default::default()
                                    },
                                    auto_forward_metadata,
                                )?;
                            ...
                            if let Some(ref token_str) = token {
                                ...
                                insert_hec_metadata(
                                    ...
                                    log,
                                    HecMetadata {
                                        token: Some(Value::from(token_str.clone())),
                                        ..Default::default()
                                    },
                                    false,
                                )?;
                                ...
                                if store_hec_token {
                                    event
                                        .metadata_mut()
                                        .set_splunk_hec_token(token_str.clone().into());
                                }
                            }
                            ...
                            if let Some(ref time_str) = query_params.time {
                                let log = event.as_mut_log();
                                if let Some(timestamp) = parse_query_timestamp(time_str) {
                                    insert_hec_metadata(
                                        log_namespace,
                                        log,
                                        HecMetadata {
                                            timestamp: Some(Value::from(timestamp)),
                                            ..Default::default()
                                        },
                                        false,
                                    )?;
    ...
    fn health_service(&self) -> BoxedFilter<(Response,)> {
        ...
        warp::get()
            ...
            .and(warp::query::<HealthQueryParams>().or_else(|_| async {
                ...
                move |header_token: Option<String>, url_token: Option<String>| {
                    ...
                    async move {
                        ...
                        let token = if allow_query_string_auth {
                            header_token.or_else(|| url_token.map(|t| format!("Splunk {}", t)))
                        } else {
                            header_token
                        };
                        ...
                        match (token, valid_credentials.is_empty()) {
                            ...
                            // Invalid token
                            (Some(_), false) => Err(Rejection::from(ApiError::InvalidToken)),
...
struct EventIteratorGenerator<'de, R: JsonRead<'de>> {
    ...
    query_sourcetype: Option<String>,
    ...
    fn from(f: EventIteratorGenerator<'de, R>) -> Self {
        Self {
            ...
            time: f
                .query_time
                .as_deref()
                .and_then(parse_query_timestamp)
                .map_or_else(|| Time::Now(Utc::now()), Time::Provided),
            extractors: [
                DefaultExtractor::new_with(
                    "host",
                    HecMetadataField::Host,
                    f.query_host
                        .or(f.remote_addr)
                ...
                DefaultExtractor::new_with(
                    "index",
                    HecMetadataField::Index,
                    f.query_index.map(Value::from),
                    f.log_namespace,
                ),
                DefaultExtractor::new_with(
                    "source",
                    HecMetadataField::Source,
                    f.query_source.map(Value::from),
                    f.log_namespace,
                ),
                DefaultExtractor::new_with(
                    "sourcetype",
                    HecMetadataField::Sourcetype,
                    f.query_sourcetype.map(Value::from),
                    f.log_namespace,
                ),
            ],
            batch: f.batch,
...
impl<'de, R: JsonRead<'de>> EventIterator<'de, R> {
    fn build_event(&mut self, mut json: JsonValue) -> Result<Event, Rejection> {
        ...
        if let Some(JsonValue::String(guid)) = json.get_mut("channel").map(JsonValue::take) {
            insert_hec_source_metadata(
                self.log_namespace,
                &mut log,
                HecMetadataField::Channel,
                Value::from(guid),
            );
        ...
        if self.auto_forward_metadata {
            ...
            for extractor in &self.extractors {
                if let Some(value) = extractor.value.clone() {
                    insert_auto_forward_metadata_field(&mut log, extractor.metadata_field, value)?;
                }
            }
        }
...
fn parse_query_timestamp(time: &str) -> Option<DateTime<Utc>> {
    if let Ok(time_f64) = time.parse::<f64>() {
        let secs = time_f64.floor() as i64;
        let nsecs = (time_f64.fract() * 1_000_000_000.0) as u32;
        Utc.timestamp_opt(secs, nsecs).single()
    } else if let Ok(time_i64) = time.parse::<i64>() {
        parse_timestamp(time_i64)
    } else {
        None
    }
}
...
fn insert_hec_source_metadata(
    log_namespace: LogNamespace,
    log: &mut LogEvent,
    field: HecMetadataField,
    value: Value,
) {
    match field {
        HecMetadataField::Channel => log_namespace.insert_source_metadata(
            SplunkHecFullConfig::NAME,
            log,
            Some(LegacyKey::Overwrite(&owned_value_path!(CHANNEL))),
            lookup::path!("channel"),
        ...
        HecMetadataField::Host => log_namespace.insert_source_metadata(
            SplunkHecFullConfig::NAME,
            log,
        ...
        HecMetadataField::Index => log_namespace.insert_source_metadata(
            SplunkHecFullConfig::NAME,
            log,
            Some(LegacyKey::Overwrite(&owned_value_path!(INDEX))),
            lookup::path!("index"),
        ...
        HecMetadataField::Source => log_namespace.insert_source_metadata(
            SplunkHecFullConfig::NAME,
            log,
            Some(LegacyKey::Overwrite(&owned_value_path!(SOURCE))),
            lookup::path!("source"),
        ...
        HecMetadataField::Sourcetype => log_namespace.insert_source_metadata(
            SplunkHecFullConfig::NAME,
            log,
            Some(LegacyKey::Overwrite(&owned_value_path!(SOURCETYPE))),
        ...
        HecMetadataField::Endpoint => log_namespace.insert_source_metadata(
            SplunkHecFullConfig::NAME,
            log,
        ...
        HecMetadataField::Token => log_namespace.insert_source_metadata(
            SplunkHecFullConfig::NAME,
            log,
            Some(LegacyKey::Overwrite(&owned_value_path!(
                "splunk_hec_full_meta",
        ...
        HecMetadataField::Timestamp => log_namespace.insert_source_metadata(
            SplunkHecFullConfig::NAME,
            log,
...
fn insert_auto_forward_metadata_field(
    log: &mut LogEvent,
    field: HecMetadataField,
    value: Value,
) -> Result<(), Rejection> {
    let target_path = match field {
        HecMetadataField::Host => event_path!("host"),
        HecMetadataField::Index => event_path!("index"),
        HecMetadataField::Source => event_path!("source"),
        HecMetadataField::Sourcetype => event_path!("sourcetype"),
        HecMetadataField::Channel
        | HecMetadataField::Endpoint
        | HecMetadataField::Token
        | HecMetadataField::Timestamp => return Ok(()),
    };
    ...
    log.insert(target_path, value);
    Ok(())
}

fn insert_raw_network_host(log_namespace: LogNamespace, log: &mut LogEvent, value: Value) {
...
fn insert_hec_metadata(
    log_namespace: LogNamespace,
    log: &mut LogEvent,
    metadata: HecMetadata,
    auto_forward_metadata: bool,
) -> Result<(), Rejection> {
    let fields = [
        (HecMetadataField::Channel, metadata.channel),
        (HecMetadataField::Host, metadata.host),
        (HecMetadataField::Index, metadata.index),
        (HecMetadataField::Source, metadata.source),
        (HecMetadataField::Sourcetype, metadata.sourcetype),
        (HecMetadataField::Endpoint, metadata.endpoint),
        (HecMetadataField::Token, metadata.token),
        (HecMetadataField::Timestamp, metadata.timestamp),
    ];

    for (field, value) in fields {
        if let Some(value) = value {
            insert_hec_source_metadata(log_namespace, log, field, value.clone());

            if auto_forward_metadata {
                insert_auto_forward_metadata_field(log, field, value)?;
            }
        }
    }
...
struct DefaultExtractor {
    field: &'static str,
    metadata_field: HecMetadataField,
    value: Option<Value>,
    log_namespace: LogNamespace,
}
...
impl DefaultExtractor {
    ...
    fn extract(&mut self, log: &mut LogEvent, value: &mut JsonValue) {
        ...
        if let Some(value) = self.value.as_ref() {
            insert_hec_source_metadata(self.log_namespace, log, self.metadata_field, value.clone());
        }
    }
}
...
fn raw_event(
    ...
    let message: Value = if gzip {
        match CappedDecoder::gzip(bytes.reader()).decompress() {
            ...
            Err(error) => {
                let rejection =
                    gzip_decompression_rejection(&error, ApiError::InvalidDataFormat { event: 0 });
    ...
    if let Some(host) = host {
        insert_raw_network_host(log_namespace, &mut log, Value::from(host));
    }

    let now = Utc::now();
    log_namespace.insert_standard_vector_source_metadata(&mut log, SplunkHecFullConfig::NAME, now);
    insert_hec_source_metadata(
        log_namespace,
        &mut log,
        HecMetadataField::Timestamp,
...
/// Each non-empty line becomes a separate event.
#[allow(clippy::too_many_arguments)]
fn raw_events_split(
    bytes: Bytes,
    gzip: bool,
    ...
    // Split by newlines without first materializing a second, potentially huge
    // line-pointer vector. Bound fan-out independently of the body-size cap.
    ...
    let mut events = Vec::new();
    for (event_index, line) in std::iter::once(first_line)
        .chain(std::iter::once(second_line))
        .chain(lines)
        .enumerate()
    {
        if event_index >= MAX_RAW_LINE_EVENTS_PER_REQUEST {
            return Err(ApiError::InvalidDataFormat { event: event_index }.into());
        }
        ...
        if let Some(ref host) = host {
            insert_raw_network_host(log_namespace, &mut log, Value::from(host.clone()));
        }
        ...
        match self.error {
            ...
            | ApiError::InvalidToken => {
                error!(
                    ...
                    error = ?self.error,
                    error_type = vector_lib::internal_event::error_type::AUTHENTICATION_FAILED,
...
fn gzip_decompression_rejection(error: &std::io::Error, fallback: ApiError) -> Rejection {
    if is_decompressed_size_limit_error(error) {
        let max = max_decompressed_size_bytes();
        warp::reject::custom(ErrorMessage::new(
            StatusCode::PAYLOAD_TOO_LARGE,
            format!("Decompressed gzip body exceeds limit of {max} bytes."),
        ))
    } else {
        fallback.into()
    }
...
fn diagnostic_body(
    ...
    if gzip {
        match CappedDecoder::gzip(body.clone().reader()).decompress() {
            Ok(data) => Some(String::from_utf8_lossy(data.as_slice()).into_owned()),
...
mod tests {
    ...
    use crate::{
        ...
        event::Event,
        sources::util::decompression::DecompressedSizeLimitExceeded,
        test_util::{
    ...
    #[tokio::test]
    async fn finish_err_maps_capped_body_to_payload_too_large() {
    ...
    #[tokio::test]
    async fn gzip_decompression_limit_maps_to_payload_too_large() {
        let error = std::io::Error::other(DecompressedSizeLimitExceeded);
        let rejection = gzip_decompression_rejection(&error, ApiError::BadRequest);

        let (response,) = finish_err(rejection)
            .await
            .expect("decompression-limit rejection should be recovered");
    ...
    async fn source_with(
        ...
        source_with_query_auth(token, valid_tokens, acknowledgements, store_hec_token, true).await
    }
    ...
    #[tokio::test]
    async fn malformed_gzip_event_preserves_bad_request_response() {
    ...
    #[tokio::test]
    async fn malformed_gzip_raw_preserves_invalid_data_format_response() {
        let (_source, address, _guard) = source().await;
        ...
        assert_eq!(body["invalid-event-number"], 0);
    }
    ...
    async fn url_used_when_header_missing() {
        let (source, address, _guard) =
            source_with_query_auth(Some(TOKEN.to_owned().into()), None, None, false, true).await;
    ...
    async fn header_priority_over_url_when_enabled() {
        let (source, address, _guard) =
            source_with_query_auth(None, Some(&["url-token", "header-token"]), None, true, true)
                .await;
    ...
    async fn url_ignored_when_disabled() {
        let (_source, address, _guard) =
            source_with_query_auth(Some(TOKEN.to_owned().into()), None, None, false, false).await;
    ...
    async fn test_raw_endpoint_url_metadata() {
        ...
        assert_eq!(
            log.get(event_path!("splunk_index"))
                .unwrap()
                .to_string_lossy(),
            "my_index"
        );
        assert_eq!(
            log.get(event_path!("splunk_sourcetype"))
                .unwrap()
                .to_string_lossy(),
            "my_type"
        );
        assert_eq!(
            log.get(event_path!("splunk_source"))
                .unwrap()
                .to_string_lossy(),
            "my_source"
        );
        ...
        assert_eq!(
            log.get(event_path!("index")).unwrap().to_string_lossy(),
            "my_index"
        );
        assert_eq!(
            log.get(event_path!("sourcetype"))
                .unwrap()
                .to_string_lossy(),
            "my_type"
        );
        assert_eq!(
            log.get(event_path!("source")).unwrap().to_string_lossy(),
            "my_source"
        );
    }
    ...
    async fn event_endpoint_metadata_vector_namespace() {
        ...
        assert!(matches!(
            log.get(vrl::metadata_path!("splunk_hec_full", "timestamp")),
    ...
    async fn raw_endpoint_metadata_vector_namespace() {
        ...
        let resp = reqwest::Client::new()
            .post(format!(
                "http://{address}/services/collector/raw?channel=ch1"
            ))
            .header("Authorization", format!("Splunk {TOKEN}"))
    ...
    async fn raw_endpoint_vector_namespace_metadata_preserved() {
        ...
        assert_eq!(
            send_req(
                address,
                "raw?channel=ch-test-meta",
                "raw-body-for-metadata",
                TOKEN,
                None,
                &[]
            )
        ...
        assert_eq!(
            log.get(vrl::metadata_path!("splunk_hec_full", "channel"))
                .unwrap()
                .to_string_lossy(),
            "ch-test-meta"
        );
    ...
    #[tokio::test]
    async fn raw_query_host_overrides_network_host_in_legacy_namespace() {
        let (source, address, _guard) = source_with_full_opts_ns(
            Some(TOKEN.to_owned().into()),
            None,
            None,
        ...
        let resp = reqwest::Client::new()
            .post(format!(
                "http://{address}/services/collector/raw?channel=ch1&host=query-host"
            ))
            .header("Authorization", format!("Splunk {TOKEN}"))
            .header("X-Forwarded-For", "192.0.2.10")
            .body("raw host precedence")
            .send()
            .await
            .unwrap();
        assert_eq!(resp.status(), 200);
        ...
        let log = collect_n(source, 1).await.remove(0).into_log();
        assert_eq!(
            log.get(event_path!("host")).unwrap().to_string_lossy(),
            "query-host"
        );
    }
    ...
    async fn auto_forward_metadata_defaults_disabled() {
        ...
        let resp = reqwest::Client::new()
            .post(format!(
                "http://{address}/services/collector/event?index=foo"
            ))
            .header("Authorization", format!("Splunk {TOKEN}"))
            .header("x-splunk-request-channel", "ch1")
    ...
    #[tokio::test]
    async fn event_body_metadata_uses_vector_namespace_without_event_field_pollution() {
        let (source, address, _guard) = source_with_full_opts_ns_auto(
            Some(TOKEN.to_owned().into()),
            None,
            None,
        ...
        let resp = reqwest::Client::new()
            ...
            .header("x-splunk-request-channel", "ch1")
            .body(
                r#"{"event":"metadata body","host":"body-host","index":"body-index","source":"body-source","sourcetype":"body-type"}"#,
            )
            .send()
            .await
            .unwrap();
        ...
        assert_eq!(
            log.get(vrl::metadata_path!("splunk_hec_full", "host"))
                .unwrap()
                .to_string_lossy(),
            "body-host"
        );
        ...
        assert_eq!(
            log.get(vrl::metadata_path!("splunk_hec_full", "source"))
                .unwrap()
                .to_string_lossy(),
            "body-source"
        );
        assert_eq!(
            log.get(vrl::metadata_path!("splunk_hec_full", "sourcetype"))
                .unwrap()
                .to_string_lossy(),
            "body-type"
        );
        ...
        assert!(log.get(event_path!("index")).is_none());
        assert!(log.get(event_path!("source")).is_none());
        assert!(log.get(event_path!("sourcetype")).is_none());
    }

    #[tokio::test]
    async fn raw_query_metadata_uses_vector_namespace_without_event_field_pollution() {
        let (source, address, _guard) = source_with_full_opts_ns_auto(
            Some(TOKEN.to_owned().into()),
            None,
            None,
        ...
        let resp = reqwest::Client::new()
            .post(format!(
                "http://{address}/services/collector/raw?channel=ch1&host=raw-host&index=raw-index&source=raw-source&sourcetype=raw-type"
            ))
            .header("Authorization", format!("Splunk {TOKEN}"))
            .body("raw metadata body")
            .send()
            .await
            .unwrap();
        assert_eq!(resp.status(), 200);
        ...
        assert_eq!(log.value().to_string_lossy(), "raw metadata body");
        assert_eq!(
            log.get(vrl::metadata_path!("splunk_hec_full", "host"))
                .unwrap()
                .to_string_lossy(),
    ...
    #[tokio::test]
    async fn event_query_metadata_uses_vector_namespace_without_event_field_pollution() {
        let (source, address, _guard) = source_with_full_opts_ns_auto(
            Some(TOKEN.to_owned().into()),
            None,
            None,
        ...
        let resp = reqwest::Client::new()
            .post(format!(
                "http://{address}/services/collector/event?channel=ch1&host=query-host&index=query-index&source=query-source&sourcetype=query-type&time=1609459200"
            ))
            .header("Authorization", format!("Splunk {TOKEN}"))
            .body(r#"{"event":"event query metadata"}"#)
            .send()
            .await
            .unwrap();
        assert_eq!(resp.status(), 200);
        ...
        for (field, expected) in [
            ...
            assert_eq!(
                log.get(vrl::metadata_path!("splunk_hec_full", field))
                    .unwrap()
                    .to_string_lossy(),
                expected
            );
            assert!(log.get(event_path!(field)).is_none());
        }
    ...
    #[tokio::test]
    async fn event_envelope_metadata_overrides_query_defaults() {
        let (source, address, _guard) = source_with_full_opts_ns_auto(
            Some(TOKEN.to_owned().into()),
            None,
            None,
        ...
        let resp = reqwest::Client::new()
            ...
            .header("Authorization", format!("Splunk {TOKEN}"))
            .body(
                r#"{"event":"envelope metadata","host":"body-host","index":"body-index","source":"body-source","sourcetype":"body-type","time":1609459201}"#,
            )
            .send()
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn auto_forward_metadata_enabled_keeps_legacy_event_fields() {
        let (source, address, _guard) = source_with_full_opts_ns_auto(
            Some(TOKEN.to_owned().into()),
            None,
            None,
        ...
        let resp = reqwest::Client::new()
            ...
            .header("x-splunk-request-channel", "ch1")
            .body(
                r#"{"event":"metadata legacy","host":"legacy-host","index":"legacy-index","source":"legacy-source","sourcetype":"legacy-type"}"#,
            )
            .send()
            .await
            .unwrap();
    ...
    async fn test_raw_endpoint_time_param() {
        ...
        let ts = log.get(event_path!("timestamp")).unwrap();
    ...
    async fn test_event_auto_extract_timestamp_param() {
        ...
        let log = events[0].as_log();
        assert_eq!(
            log.get(event_path!("auto_extract_timestamp"))
                .unwrap()
                .to_string_lossy(),
            "true"
        );
    ...
    async fn test_event_no_auto_extract_timestamp() {
        ...
        let events = collect_n(source, 1).await;
        let log = events[0].as_log();
        assert!(log.get(event_path!("auto_extract_timestamp")).is_none());
    }
    ...
    async fn test_raw_without_url_metadata_preserves_body() {
        let (source, address, _guard) =
            source_with_full(Some(TOKEN.to_owned().into()), None, None, false, false).await;
    ...
    async fn test_basic_auth_rfc1945() {
        ...
        assert_eq!(
            resp.status(),
            200,
            "Basic auth (RFC 1945) should be accepted"
        );
    ...
    async fn test_batch_mixed_string_object_events() {
        let (source, address, _guard) = source().await;
        let batch =
            r#"{"event":"string event"}{"event":{"key":"object event"}}{"event":"another string"}"#;
        ...
        assert!(
            log0.get(event_path!("message")).is_some(),
        ...
        assert_eq!(
            log1.get(event_path!("key")).unwrap().to_string_lossy(),
            "object event"
        );
        ...
        assert!(
            log2.get(event_path!("message")).is_some(),
            "String event should have 'message' field"
        );
    }
    ...
    async fn integer_event_preserved_with_url_metadata() {
        ...
        let msg = log
            ...
            .expect("Integer event should be wrapped in 'message' field");
        assert_eq!(
            msg,
            &Value::Integer(42),
            "Integer value should be preserved as 42"
        );
    }
    ...
    async fn float_event_preserved_with_url_metadata() {
        ...
        let msg = log
            .get(event_path!("message"))
            .expect("Float event should be wrapped in 'message' field");
    ...
    async fn boolean_event_preserved_with_url_metadata() {
        ...
        let msg = log
            .get(event_path!("message"))
            .expect("Boolean event should be wrapped in 'message' field");
        assert_eq!(
            msg,
            &Value::Boolean(true),
    ...
    async fn array_event_preserved_with_url_metadata() {
        ...
        let msg = log
            .get(event_path!("message"))
            .expect("Array event should be wrapped in 'message' field");
    ...
    async fn test_batch_mixed_types_with_bool_rejected_whole_batch() {
        let (source, address, _guard) = source().await;
        let batch =
            r#"{"event":"string val"}{"event":42}{"event":true}{"event":{"k":"v"}}{"event":[1,2]}"#;
        let resp = send_req(address, "event", batch, TOKEN, Some("ch1"), &[]).await;
        assert_eq!(resp.status(), 400);
        let body: serde_json::Value = resp.json().await.unwrap();
        assert_eq!(body["code"], 6);
    ...
    async fn test_raw_line_splitting() {
        ...
        assert_eq!(
            events.len(),
            3,
            "Multiline raw body should produce 3 events"
        );
    ...
    #[test]
    fn raw_line_splitting_rejects_excessive_fanout() {
        let body = "x\n".repeat(MAX_RAW_LINE_EVENTS_PER_REQUEST + 1);
        let events_received = register!(EventsReceived);
        let rejection = raw_events_split(
            Bytes::from(body),
            false,
            "ch1".to_owned(),
            None,
        ...
        .expect_err("excessive raw line fan-out must be rejected");

        assert!(matches!(
            rejection.find::<ApiError>(),
            Some(ApiError::InvalidDataFormat { event })
                if *event == MAX_RAW_LINE_EVENTS_PER_REQUEST
        ));
    }
    ...
    async fn test_raw_line_splitting_disabled() {
        ...
        assert_eq!(
            events.len(),
            1,
            "With splitting disabled, should be 1 event"
        );
        ...
        assert!(
            msg.contains("line_A"),
            "Single event should contain all text"
        );
    ...
    async fn paths_raw_1_0() {
        ...
        let resp = reqwest::Client::new()
            .post(format!(
                "http://{address}/services/collector/raw/1.0?channel=ch1"
            ))
            .header("Authorization", format!("Splunk {TOKEN}"))
            .body("alias raw 1.0")
            .send()
